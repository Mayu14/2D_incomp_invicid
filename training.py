...
# (形状)_(データ数)とする
def get_case_number_beta(case_number, rr, sr, skiptype, shape_data=200, total_data=200000, reductioning=False, inputNormalize=False,
...
def main(fname_lift_train, fname_shape_train, fname_lift_test, fname_shape_test, case_number, case_type=3, env="Lab"):
    ...
    for sr in s_rate:
        for rr in r_rate:
            ...
            with tf.Graph().as_default():
                ...
                if env == "Lab":
                    ...
                    case_num = get_case_number_beta(case_number, rr, sr, s_skiptype, reductioning = reductioning,
                                                    inputNormalize=inputNormalization, unballanced = unballanced, outputNormalize=outputNormalization)
                ...
                if case_type == 3:
                    X_train, y_train = read_csv_type3(source, fname_lift_train, fname_shape_train, shape_odd = s_odd, read_rate = rr, skip_rate=sr, skip_angle = s_skiptype, unbalance = unballanced)
                ...
                if (sr == 1) and (reductioning):
                    X_train, y_train = data_reduction(X_train, y_train, reduction_target = 25000, preprocess = "PCA")
                    ...
                    if useSequential:
                        ...
                        def regressor(input_dim):
                            ...
                            x = Dense(units = 51, activation = None, kernel_initializer = rand_init,
                                      kernel_regularizer = reg, bias_regularizer = reg, name = "dense_1")(x)
                            x = BatchNormalization(axis = -1)(x)
                            x = PReLU(shared_axes = [1])(x)
                            x = Dropout(0.25)(x)
                        ...
                        """
                        model = regressor(input_vector_dim)
                ...
                chkpt = baseSaveDir + 'incomp2MLP_.{epoch:02d}-{val_loss:.2f}.hdf5'
                ...
                y_predict = scalar2.inverse_transform(model.predict(x_test))
                y_test = scalar2.inverse_transform(y_test)
            ...
            score_test = model.evaluate(x_test, scalar2.transform(y_test), verbose = 0)
            r2_test = r2_score(y_test, y_predict)
            maxE = (y_predict - y_test)[np.argmax(np.abs(y_predict - y_test))]
            minE = (y_predict - y_test)[np.argmin(np.abs(y_predict - y_test))]
            print(r2_test, score_test[0], score_test[1])
            print(maxE, minE)
            log = [r2_test, score_test[0], score_test[1], maxE, minE]
            log.extend(case_num.split("_"))
            with open(baseSaveDir + "incomplog2.csv", "a") as f:
