...
class ArcLength():
    def __init__(self, x=None, y=None, normalize=1, min_dif =10e-6,
                 ...
                 encoded_obj=None, overwrite_bn=True, onlyArc=False):
        # common
    ...
    def getArcXY(self):
        s = np.linspace(start = 0, stop = 1, num = self.n_max)
        self.arcX = self.fx(s)
        self.arcY = self.fy(s)
        self.arcXY = np.concatenate([self.arcX, self.arcY])
...
def make_circum_arc_from_naca(naca4, resolution, digit=4):
    if digit == 4:
    ...
    naca.one_stroke_ccw()
    arc = ArcLength(x=naca.ccw_x, y=naca.ccw_y, onlyArc=True)
    return arc.arcXY
