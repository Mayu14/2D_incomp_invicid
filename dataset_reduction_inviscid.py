...
def data_reduction(X_data, y_data, reduction_target = 10000, output_csv = False, preprocess = "None", update_xy = False,
                   criteria_method = "nearest_centroid", main_process = "kmeans++", cv_types = "all",
                   check_plot = False, force_overwrite = False,
                   fname = "reduct",
                   path = "G:\\Toyota\\Data\\Incompressible_Invicid\\training_data\\NACA4\\reduct_csv\\", debug_mode=False):
    # X_data:[N_samples, M_features]
    # y_data:[N_samples, K_features]
    ...
    def reduction2k_datas(X_data, y_data, k_cluster, indices = None, output_indices = True,
                          criteria_method = "nearest_centroid", main_process = "kmeans++"):
    ...
    def preprocessing(X_data, preprocess, force_overwrite = False):
        ...
        save_path = "G:\\Toyota\\Data\\Incompressible_Invicid\\training_data\\NACA4\\reduct_csv\\"
    ...
    if type(nearest_indices) == type(None):
        ...
        print(X_data)
    X_data, y_data, nearest_indices = reduction2k_datas(X_data = X_data, y_data = y_data, k_cluster = reduction_target,
                                                        criteria_method = criteria_method, indices = nearest_indices,
    ...
    def make_plot(nearest_indices, X_origin, y_origin, preprocess, criteria_method, main_process, fname):
        ...
        X_data4plot, y_data4plot, indices = reduction2k_datas(X_data = X_origin, y_data = y_origin,
                                                              k_cluster = reduction_target, output_indices = False,
                                                              criteria_method = criteria_method,
                                                              indices = nearest_indices, main_process = main_process)
        ...
        for i in range(2):
            ...
            ax.plot(transformed[:, 0], transformed[:, 1], color = "limegreen", marker = ".", linestyle = "None",
                    label = "reduced")
            tail = ""
            if i == 1:
                ax.plot(tr_origin[:, 0], tr_origin[:, 1], color = "dodgerblue", marker = ".", linestyle = "None",
                        alpha = 0.1,
                        label = "origin")
                tail += "with_original"
...
def decode():
    X_train, y_train = read_csv_type3(source, fname_lift_train, fname_shape_train, shape_odd = 0, read_rate = 1,
                                   total_data = 0)
    nearest_indices = data_reduction(X_train, y_train, preprocess = "PCA", reduction_target = 25000,
                                     debug_mode = True)
    X_train, gomi = read_csv_type3(source, fname_lift_train, fname_shape_train, shape_odd = 0, read_rate = 1,
                                   total_data = 0, debug_mode = True)
    #print(X_train)
    print(X_train["naca4"].drop_duplicates())
    print(X_train["angle"].drop_duplicates())
    X_train = X_train.values
    na = X_train[nearest_indices, 0:2]
    #na[:, 0] = ((na[:, 0] + 1) / 2)
    #na[:, 1] = (na[:, 1] + 40) / 2
    fig = plt.figure()
    ax = fig.add_subplot(111)
    
    ax.plot(na[:, 0], na[:,1], ".", linewidth=0.0, markersize=1)
    plt.show()
    import pandas as pd
    df = pd.DataFrame(na, columns = ["naca4", "angle"])
    df.astype(int)
    print(df)
    exit()
...
if __name__ == '__main__':
    ...
    # main_process = "kmeans++"
    preprocesses = ["PCA"]  # , "PCA", "rbf", "poly", "linear", "cosine", "sigmoid"]
    # preprocesses = ["None"]
    postprocesses = ["farthest_from_center"]#"nearest_centroid"]  # , "farthest_from_center"]

    for preproc in preprocesses:
        for mainproc in main_processes:
            for postproc in postprocesses:
                cluster = 25000
                data_reduction(X_train, y_train, preprocess = preproc, reduction_target = cluster,
                               output_csv = True, main_process = mainproc, criteria_method = postproc,
                               cv_types = "tied", check_plot = False, force_overwrite = False)
