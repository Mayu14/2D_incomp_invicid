...
class CGAN():
    def __init__(self):
        ...
        """
        plot_model(self.discriminator, to_file="model_discriminator.png", show_shapes=True)
        plot_model(self.generator, to_file="model_generator.png", show_shapes=True)
        plot_model(self.combined, to_file="model_combined.png", show_shapes=True)
        exit()
        """
    ...
    def combine_images(self, generated_images):
        ...
        if SHAPE_REPRESENTATION == "modified_fourier":
            for index, image in enumerate(generated_images):
                ...
                image = get_decode_img_from_bnxy(image[:, 0, 0][1:], from_concat_bn_xy = True, angle = image[:, 0, 0][0], imgWidth=WIDTH, imgHeight=HEIGHT, deg=True)
            ...
            for index in range(num_batches):
                ...
                if epoch % 100 == 0:
                    ...
                    if not os.path.exists(self.path):
                        os.mkdir(self.path)
                    Image.fromarray(image.astype(np.uint8)).save(self.path + CASE_HEADER + "%04d_%04d.png" % (epoch, index))
