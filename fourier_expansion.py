...
class fourier_expansion(object):
    ...
    def test_plot_decryption_data(self, ylim=[-0.25,0.25]):
...
def main():
    from naca_4digit_test import Naca_5_digit
    #naca = Naca_4_digit(int_4 = "6130", attack_angle_deg = 0, resolution = 100)
    naca = Naca_5_digit(int_5="21019", attack_angle_deg=0, resolution=100)
    naca.plot()
    plt.plot(naca.x_l, naca.y_l, "x")
    plt.plot(naca.x_u, naca.y_u, "x")
    #plt.plot(naca.equidistant_x, naca.equidistant_y_l)
    #plt.plot(naca.equidistant_x, naca.equidistant_y_u)
    plt.xlim([-0.1,1.1])
    plt.ylim([0.0,1.0])
    plt.show()
    fex = fourier_expansion(naca.x_u, naca.y_u, naca.x_l, naca.y_l, n=200)
    # fex.test_plot_raw_data()
    fex.test_plot_decryption_data(ylim=[-0.5,0.5])
